...
const InstructionItem: React.FC<InstructionItemProps> = ({ index, instruction }) => {
  ...
  return (
    <li className="px-3 py-3 mb-2 bg-neutral-100 rounded-xs text-black gap-3 flex flex-row">
      {/* upvote */}
      <p className="flex flex-col gap-0.5 text-gray-400 text-xs">
        <span onClick={() => handleUpvote(instruction.id)}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4 cursor-pointer hover:text-purple-600 hover:font-semibold">
            <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" />
          </svg>
        </span>
        <span className="text-sm font-semibold text-center text-gray-400 ">
          {instruction.votes < 10 ? `0${instruction.votes}` : instruction.votes}
        </span>
        <span onClick={() => handleDownvote(instruction.id)}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4 cursor-pointer hover:text-purple-600 hover:font-semibold">
      ...
      <div className="flex flex-col gap-1 flex-grow cursor-pointer" onClick={toggleExpand} >
        ...
        <p className="flex w-full font-semibold text-xs text-gray-400 justify-right">@{instruction.creator.split('@')[0]}</p>
        <p className={`text-xs text-gray-600 ${isExpanded ? '' : 'line-clamp-3'}`}>
          {instruction.input}
        </p>
        <span className="text-xs text-purple-600/70 cursor-pointer">
